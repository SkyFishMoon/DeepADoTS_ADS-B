...
class LSTMVED_Local(Algorithm, PyTorchUtils):
    def __init__(self, name: str = 'LSTM-VED-Local', num_epochs: int = 10, batch_size: int = 20, lr: float = 1e-3,
    ...
    def fit(self, X: pd.DataFrame, tensorboard):
        ...
        self.lstm_ved_local = LSTMVEDModule(X.shape[1], self.hidden_size,
                                   self.n_layers, self.use_bias, self.dropout,
                                   seed=self.seed, gpu=self.gpu, latent_length=self.latent_length)
        self.to_device(self.lstm_ved_local)
        self.optimizer = torch.optim.Adam(self.lstm_ved_local.parameters(), lr=self.lr)
        self.scheduler = lr_scheduler.ExponentialLR(self.optimizer, gamma=0.8)

        self.lstm_ved_local.train()
        train_step = 0
        val_step = 0
        for epoch in trange(self.num_epochs):
            ...
            for ts_batch in train_loader:
                ...
                start = time.time()
                output = self.lstm_ved_local(self.to_var(ts_batch))
                loss = nn.MSELoss(size_average=False)(output, self.to_var(ts_batch.float()))
                kl_loss = -0.5 * torch.mean(1 + self.lstm_ved_local.lmbda.latent_logvar -
                            self.lstm_ved_local.lmbda.latent_mean.pow(2) -
                                self.lstm_ved_local.lmbda.latent_logvar.exp())
                loss = loss + kl_loss
                # tensorboard.add_scalar("learning_rate/", self.lr, train_step)
                self.lstm_ved_local.zero_grad()
                loss.backward()
                torch.cuda.synchronize()
                end = time.time()
            ...
            if epoch % 1 == 0:
                ...
                torch.save({'epoch': epoch, 'state_dict': self.lstm_ved_local.state_dict(),
                            'optimizer': self.optimizer.state_dict(),
                            'scheduler': self.scheduler.state_dict()
                            }, self.ckpt_dir + 'epoch' + str(epoch) + '.pth')
                val_step = val_step + 1

        self.lstm_ved_local.eval()
        error_vectors = []
        for ts_batch in train_gaussian_loader:
            output = self.lstm_ved_local(self.to_var(ts_batch))
    ...
    def predict(self, X: pd.DataFrame, tensorboard):
        ...
        for idx, ts in enumerate(data_loader):
            with torch.no_grad():
                output = self.lstm_ved_local(self.to_var(ts))
...
class LSTMVEDModule(nn.Module, PyTorchUtils):
    def __init__(self, n_features: int, hidden_size: int,
        ...
        self.decoder = nn.LSTM(self.latent_length, self.hidden_size, batch_first=True,
    ...
    def forward(self, ts_batch, return_latent: bool = False):
        ...
        memory_bank, enc_hidden = self.encoder(ts_batch.float(), enc_hidden)  # .float() here or .double() for the model
        # h, c = enc_hidden
        # 2. Use hidden state as initialization for our Decoder-LSTM
        memory_bank_z = self.lmbda(memory_bank)
        dec_hidden = self._init_hidden(batch_size)
        output = self.hidden2output(self.decoder(memory_bank_z, dec_hidden)[0])
