...
class TransformerVED_Local(Algorithm, PyTorchUtils):
    def __init__(self, name: str = 'Transformer-VED-Local', num_epochs: int = 10, batch_size: int = 20, lr: float = 1e-3,
        ...
        self.transformer_ved_local = None
    ...
    def fit(self, X: pd.DataFrame, tensorboard):
        ...
        self.transformer_ved_local = TransformerVEDModule(X.shape[1], self.hidden_size,
        ...
        self.to_device(self.transformer_ved_local)
        self.optimizer = torch.optim.Adam(self.transformer_ved_local.parameters(), lr=self.lr)
        def warm_decay(step):
            if step < self.warmup:
        ...
        self.transformer_ved_local.train()
        train_step = 0
        val_step = 0
        for epoch in trange(self.num_epochs):
            ...
            for ts_batch in train_loader:
                ...
                start = time.time()
                output = self.transformer_ved_local(self.to_var(ts_batch))
                loss = nn.MSELoss(size_average=False)(output, self.to_var(ts_batch.float()))
                kl_loss = -0.5 * torch.mean(1 + self.transformer_ved_local.lmbda.latent_logvar -
                                            self.transformer_ved_local.lmbda.latent_mean.pow(2) -
                                            self.transformer_ved_local.lmbda.latent_logvar.exp())
                loss = loss + kl_loss
                self.transformer_ved_local.zero_grad()
                loss.backward()
                torch.cuda.synchronize()
                end = time.time()
            ...
            if epoch % 1 == 0:
                ...
                torch.save({'epoch': epoch, 'state_dict': self.transformer_ved_local.state_dict(),
                            'optimizer': self.optimizer.state_dict(),
                            'scheduler': self.scheduler.state_dict()
                            }, self.ckpt_dir + '-epoch' + str(epoch) + '.pth')
                val_step = val_step + 1

        self.transformer_ved_local.eval()
        error_vectors = []
        for ts_batch in train_gaussian_loader:
            output = self.transformer_ved_local(self.to_var(ts_batch))
    ...
    def predict(self, X: pd.DataFrame, tensorboard):
        ...
        for idx, ts in enumerate(data_loader):
            with torch.no_grad():
                output = self.transformer_ved_local(self.to_var(ts))
...
class TransformerVEDModule(nn.Module, PyTorchUtils):
    ...
    def forward(self, ts_batch, return_latent: bool = False):
        ...
        # if self.training:
        z = self.lmbda(memory_bank)
        tgt_input = self.position_encoding(torch.flip(z, dims=[1]))
        ...
        output = self.hidden2output(tgt_output)
        # else:
