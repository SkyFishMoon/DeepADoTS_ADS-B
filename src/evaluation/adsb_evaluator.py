...
class ADSBEvaluator:
    ...
    def init_detector(self):
        return model[self.opt.name].from_opt(self.opt, self.seed)
    ...
    def evaluate(self):
        ...
        try:
            ...
            self.results[('ADS-B', self.detector.name)] = np.zeros_like(y_test)
        # for ds in progressbar.progressbar(self.datasets):
    ...
    def get_metrics_by_thresholds(self, y_test: list, score: list, thresholds: list):
        for threshold in thresholds:
            ...
            metrics = ADSBEvaluator.get_accuracy_precision_recall_fscore(y_test, anomaly)
    ...
    def create_bar_charts(self, runs, detectorwise=False, store=True):
        target = 'algorithm' if detectorwise else 'dataset'
        grouped_by = 'dataset' if detectorwise else 'algorithm'
    ...
    def store(self, fig, title, extension='pdf', no_counters=False, store_in_figures=False):
        ...
        os.makedirs(output_dir, exist_ok=True)
        counters_str = '' if no_counters else f'--{len(self.datasets)}'
    ...
    def store_text(self, content, title, extension='txt'):
        ...
        path = os.path.join(output_dir, f'{title}--{len(self.datasets)}-{timestamp}.{extension}')
    ...
    def print_merged_table_per_algorithm(self, results):
        ...
        table = tabulate(results[results['algorithm'] == self.detector.name], headers='keys', tablefmt='psql')
        self.logger.info(f'Detector: {self.detector.name}\n{table}')

    def gen_merged_latex_per_algorithm(self, results, title_suffix=None, store=True):
        title = f'latex_merged{f"_{title_suffix}" if title_suffix else ""}'
        content = ''
        # for det in self.detectors:
        content += f'''{self.detector.name}:\n\n{tabulate(results[results['algorithm'] == self.detector.name],
                                   headers='keys', tablefmt='latex')}\n\n'''
        if store:
            self.store_text(content=content, title=title, extension='tex')
        return content
    ...
    def get_key_and_value(dataset_name):
        ...
        return ADSBEvaluator.translate_var_key(var_key), var_value
    ...
    def insert_multi_index_yaxis(ax, mi_df):
        ...
        dataset_types = ADSBEvaluator.get_dataset_types(mi_df)  # Returns unique entries keeping original order
    ...
    def get_multi_index_dataframe(self):
        return ADSBEvaluator.to_multi_index_frame([self])

    @staticmethod
    def plot_heatmap(evaluators, store=True):
        mi_df = ADSBEvaluator.to_multi_index_frame(evaluators)
        ...
        ADSBEvaluator.insert_multi_index_yaxis(ax, mi_df)
    ...
    def plot_single_heatmap(self, store=True):
        ADSBEvaluator.plot_heatmap([self], store)
    ...
    def gen_merged_tables(self, results, title_suffix=None, store=True):
        ...
        std_results, avg_results, avg_results_renamed = ADSBEvaluator.get_printable_runs_results(results)
